            ...
            steps {
                script {
                    docker.withRegistry('https://registry.hub.docker.com', 'docker-hub-token') {
                        sh '"C:/Program Files/Git/bin/bash.exe" -c "docker build -t my-django-app ."'
                        sh '"C:/Program Files/Git/bin/bash.exe" -c "docker tag my-django-app ayushsavaliya53/my-django-app:latest"'
                        sh '"C:/Program Files/Git/bin/bash.exe" -c "docker push ayushsavaliya53/my-django-app:latest"'
                    }
                }
            }
        }
